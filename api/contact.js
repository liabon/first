// api/contact.js
// POST /api/contact
// 이메일 발송 + 업무용 드론 문의 DB 저장
//
// DB: 업무용 드론 문의만 drone_inquiries 테이블에 저장
// 개인용 드론보험 DB 저장은 /api/insurance 에서 처리 (중복 저장 없음)
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS || !process.env.ADMIN_EMAIL) {
      console.error('[contact] 이메일 환경변수 미설정');
    ...
    // ── 업무용 드론 문의 DB 저장 ─────────────────────────────
    // 개인용 드론보험 DB 저장은 /api/insurance 에서 담당 (여기서 중복 저장하지 않음)
    if (!isQuoteEmailOnly && (insurance_type === '업무용 드론보험' || request_type === 'business_quote')) {
      try {
        const db = getPool();
        await db.query(
          `INSERT INTO drone_inquiries (name, phone, email, insurance_type, message, source_page, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [
            req.body.manager_name  || name  || '',
            req.body.manager_phone || phone || '',
            req.body.manager_email || email || '',
            'business',
            req.body.inquiry || message || '',
            'drone-insurance',
            'new'
          ]
        );
        console.log('[contact] 업무용 드론 문의 DB 저장 완료');
      } catch (dbErr) {
        console.error('[contact] 업무용 드론 DB 저장 실패:', dbErr.message);
        return res.status(500).json({ message: 'DB 저장에 실패했습니다. 잠시 후 다시 시도해주세요.' });
      }
    }
    ...
    // ── 관리자 이메일 발송 ────────────────────────────────────
    // 개인용 드론보험(견적서 재발송 포함)과 quote_email은 관리자 발송 제외
    ...
    console.error('[contact] 처리 중 오류:', error);
